...
integer function HACApK_init(nd,st_ctl,st_bemv,icomma)
 ...
 st_ctl%param(8) =20;       ! 10:BLR, 20:BH, other:Leafmtx
...
!! do ill=1,10
!! ill=1; itt=1; zzz=HACApK_entry_ij(lod(ill),lod(itt),st_bemv)
!!   itt=1; zzz=HACApK_entry_ij(lod(ill),lod(itt),st_bemv)
!!   print*,'ill=',ill,'; itt=',itt,'; zzz=',zzz
!! enddo
...
!***HACApK_RRQR
 integer function HACApK_RRQR(zaa,zab,param,ndl,ndt,nstrtl,nstrtt,lod,st_bemv,kmax,eps,znrmmat,pSVD_EPS)
 type(st_HACApK_calc_entry) :: st_bemv
 real*8 :: param(:)
...
!$omp end critical
     ...
     elseif(param(60)==3)then
       kt=HACApK_SVD(zaa,zab,param,ndl,ndt,nstrtl,nstrtt,lodl,st_bemv,kparam,eps,znrmmat,ACA_EPS)
     elseif(param(60)==4)then
      ! kt=HACApK_rSVD(zaa,zab,param,ndl,ndt,nstrtl,nstrtt,lodl,st_bemv,kparam,eps,znrmmat,ACA_EPS)
     elseif(param(60)==5)then
       kt=HACApK_RRQR(zaa,zab,param,ndl,ndt,nstrtl,nstrtt,lodl,st_bemv,kparam,eps,znrmmat,ACA_EPS)
     else
       print*,'Only ACA and ACA+ is avairable! Set param(60)=1-5.'
       stop
     endif
     if(kt>kparam-1) then
!!!$omp critical
...
!***HACApK_chk_rank_residual
 subroutine HACApK_chk_rank_residual(st_lf,lod,st_bemv)
 type(st_HACApK_leafmtx) st_lf
 integer*4 :: lod(:)
 type(st_HACApK_calc_entry) :: st_bemv
 ...
 ndl=st_lf%ndl; ndt=st_lf%ndt; kt=st_lf%kt
 nstrtl=st_lf%nstrtl; nstrtt=st_lf%nstrtt
 allocate(zresk(0:kt)); zresk=0.0d0
 zanrm=0.0d0
 do il=1,ndl; ill=il+nstrtl-1
   do it=1,ndt; itt=it+nstrtt-1
     zz=HACApK_entry_ij(lod(ill),lod(itt),st_bemv)
     zanrm=zanrm+zz*zz
     do ik=1,kt
       zz=zz-st_lf%a2(il,ik)*st_lf%a1(it,ik)
...
!***HACApK_chk_leafmtx
 subroutine HACApK_chk_leafmtx(st_leafmtxp,st_ctl,lnmtx,nd,mem8,st_bemv)
 include 'mpif.h'
 type(st_HACApK_leafmtxp) :: st_leafmtxp
 type(st_HACApK_lcontrol) :: st_ctl
 type(st_HACApK_calc_entry) :: st_bemv
 ...
 do ip=1,st_leafmtxp%nlf
   ...
   if(st_leafmtxp%st_lf(ip)%ltmtx==1)then
     ...
     if((nstrtl+ndl)>nd .and. nstrtt==1) then
        ...
        call HACApK_chk_rank_residual(st_leafmtxp%st_lf(ip),st_ctl%lod,st_bemv)
     endif
