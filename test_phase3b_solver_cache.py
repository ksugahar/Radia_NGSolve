#!/usr/bin/env python
"""
Test Phase 3B: Full H-matrix serialization for SOLVER interaction matrix
Tests the complete save/load cycle for solver interaction matrix

This test focuses on the solver interaction matrix (used in relaxation/solving),
not the field evaluation matrix.
...
def create_nonlinear_problem(nx=10, ny=10):
    ...
    source = rad.ObjRecMag([25, 25, -10], [20, 20, 5], [0, 0, 795774.7])
...
def test_solver_first_run():
    ...
    # Create geometry
    container = create_nonlinear_problem(10, 10)
...
def test_solver_second_run(B_expected):
    ...
    container_new = create_nonlinear_problem(10, 10)
    ...
    # Solve
    print("\nSolving with relaxation...")
    result = rad.Solve(container_new, 0.0001, 1000)
    print(f"Solver result: {result}")
...
def test_performance(t_build, t_load):
    ...
    print(f"Target:      >10x (for solver matrix)")

    if speedup > 10:
        print(f"[OK] PASS: Achieved {speedup:.0f}x speedup (target: >10x)")
        return True
    else:
        print(f"[FAIL] FAIL: Only {speedup:.0f}x speedup (target: >10x)")
        print("\nNote: If speedup is low, the H-matrix may already be cached")
        print("      Try deleting .radia_cache/ folder to force rebuild")
        return False

def main():
    print("="*70)
    print("Phase 3B: Full H-Matrix Serialization Test (Solver Matrix)")
    print("="*70)
    print("\nThis test verifies:")
    ...
    try:
        ...
        if accuracy_pass and performance_pass:
            ...
            print("[INFO] Performance test needs investigation")
